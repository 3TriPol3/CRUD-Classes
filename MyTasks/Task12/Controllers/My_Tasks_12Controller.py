...
from MyTasks.Task12.Models.MyTasks_12 import *

class ProjectsController:
    '''
    CRUD
    Функции: добавить проект, изменить статус, проекты по приоритету, просроченные
    '''
    ...
    def add(cls, name, status, deadline, priority):
        ...
        ProjectsList.create(name=name, status=status, deadline=deadline, priority=priority)

    # Изменить статус(status) по - id
    @classmethod
    def update(cls, id, **kwargs):
        ProjectsList.update(**kwargs).where(ProjectsList.id == id).execute()

    # проекты по приоритету - priority
    @classmethod
    def get_priority(cls, priority):
        return ProjectsList.select().where(ProjectsList.priority == priority)
...
if __name__ == "__main__":
    # ProjectsController.add('Веб-сайт', 'в разработке', '2025-11-19', 'высокий')  # Добавить проект

    ProjectsController.update(1, status='Завершено') # Изменить статус

    for element in ProjectsController.get_priority('высокий'): # Проекты по приоритету
        print(element.id, element.name, element.status, element.deadline, element.priority)

    # ProjectsController.overdue_projects() # Просроченные проекты # /*/